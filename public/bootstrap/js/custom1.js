...
$("button[data-element='menu']").on("click", function () {
    ...
    $(this).siblings('.collapse').not($(this).next()).removeClass('show');
    $(this).siblings('.collapse').find('.collapse').removeClass('show');
    $(this).siblings('.collapse').find('.active').removeClass('active').children().first().removeClass('text-white');
...
$('.nav-link[data-bs-toggle="collapse"]').on('click', function() {
    ...
    if (!$(this).parent('#v-pills-tab').length) {
        ...
        if($(this).next().hasClass('panduan') && window.location.pathname === '/dashboard/petunjuk/sewa') {
            if($(this).next().children().children().hasClass('active')) {
                $(this).next().children().children('.active').children().first().addClass('text-white');
            }
        }
        if(window.location.pathname === '/dashboard/sewa') {
            if (!$(this).next().find("#panduan").children().first().hasClass('text-white')) {
                $(this).next().find("#panduan").children().first().addClass('text-white');
            }
        }

        if(window.location.pathname === '/dashboard') {
            if ($(this).parent().parent().attr('id') === 'submenu-petunjuk') {
                console.log($(this).next().children().children(':nth-last-child(2)'));
                if ($(this).next().children().children(':nth-last-child(2)').hasClass('active') && !$(this).next().children().children(':nth-last-child(2)').children().first().hasClass('text-white')) {
                    $(this).next().children().children(':nth-last-child(2)').children().first().addClass('text-white')
                }
            }
        }
    }
...
$("#sidebar-icon").on('click', function(){
    if(window.location.href.includes('/dashboard')) {
        if($(this).prev().children().first().is(':hidden')) {
            $(this).prev().children().first().show();
            $(this).prev().removeClass('col-md-3').addClass('col-md-3');
            $(this).children().removeClass('right').addClass('left');
            $(this).children().css('left', '5px');
        } else {
            $("#v-pills-tab").hide();
            $(this).prev().removeClass('col-md-3');
            $(this).children().removeClass('left').addClass('right');
            $(this).children().css('left', '1px');
        }
    }
});
