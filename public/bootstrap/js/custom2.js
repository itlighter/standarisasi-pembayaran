...
(async function () {
    if (window.location.pathname.includes('/dashboard')) {
        ...
        const stbHrefs = [
            window.location.origin + "/dashboard/sewa/2?page=stb&tab=" + encodeURIComponent(syarat),
            window.location.origin + "/dashboard/sewa/2?page=stb&tab=" + encodeURIComponent(penginputan)
        ];
        $(stb).each(function(index){
            $(this).attr('href', stbHrefs[index]);
        });
        const sstbHrefs = [
            window.location.origin + "/dashboard/sewa/2?page=sstb&tab=" + encodeURIComponent(syarat),
            window.location.origin + "/dashboard/sewa/2?page=sstb&tab=" + encodeURIComponent(penginputan)
        ];
...
$("#sidebar-icon").on("click", function() {
    if($("#v-pills-tab").is(":hidden")){
        ...
        $("#v-pills-tab").hide();
        $("#v-pills-tab").parent().prev().show();
        $("#v-pills-tab").parent().prev().css('padding-left', '5px');
        $("#tab").css('margin-left', '35px');
        $(this).children().removeClass("left");
